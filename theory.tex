...
\section*{Q7}
The code only works iff the encrypted lists are named exactly as assumed in the code and at exactly the assumed place ie in the same folder. One could get user input for the filepath but it was not mentioned in the exercise to do so so we simply won't.

The decrypted version reads: 

Professor: ``You are late in submitting your Haskell assignment''.
Student: ``In fact, I have submitted it, but you have not started evaluating it yet''.
Professor: ``Actually, I have, but I have not been asked to submit the grades yet''!

Clearly it is about Lazy eval, which is the prctice of evaluating if and only if a specific value is needed.
\end{document}
