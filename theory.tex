...
\beginn{enumerate}[(a)]
\item
Currying is the revriting of a fuction of multziple inputs as a sequennze of fuctions withj one input.
\[
f(x,y,z) \quad \text{would be rewritten to} \quad f(x)(y)(z)
\]
I.E a fuction of type 
\[
 A \times B \to C
\]
Becomes a fuction of type:
\[
A \to B\to C
\]
$\lnot$ Curried:
...
\]
So with enough motivtion any fuction could be rewritten to be curried or uncurried, altough this might be quite a hand full of work for complex fuctions.
\subsection*{Benefits}
We can quite easily fix an argument and get a new fuction like so 
\begin{verbatim}
    add(5) 
\end{verbatim}
Which gives as a fuc $y \to 5 + y $. Which makes reausing it less painfull. If we wanted to do this with the uncurried version we would need to write a hlper fuction that gives us a toupe of desiered form.
\item 
\end{enumerate}
\end{document}